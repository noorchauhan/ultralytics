...
class DetectionTrainer(BaseTrainer):
    ...
    def get_model(self, cfg: str | None = None, weights: str | None = None, verbose: bool = True):
        ...
        # Rebuild EMA-distillation model on resume.
        if isinstance(weights, DistillationModelEMA):
            if verbose and RANK == -1:
                LOGGER.info("Resuming EMA DistillationModel with rebuild-and-load from checkpoint weights")
            ...
            if verbose and RANK == -1:
                if incompatible.missing_keys:
                    LOGGER.warning(f"Missing keys when loading EMA distillation checkpoint: {incompatible.missing_keys}")
                if incompatible.unexpected_keys:
                    LOGGER.warning(f"Unexpected keys when loading EMA distillation checkpoint: {incompatible.unexpected_keys}")
            model.criterion = None
            return model

        # Rebuild distillation model on resume to avoid inheriting checkpoint module states (e.g., requires_grad flags).
        if isinstance(weights, DistillationModelStd):
